...
def msg():    #입력 값 변수 저장 함수
    global contents
    contents = ent1.get(1.0,"end-1c")
    win.destroy()


####초기 설정
win = Tk()
win.title("서울시립대학교 학습공동체 멍멍!! 우리는 웰C코기 팀 손글씨변환 프로그램")
...
#### 입력 버튼
btn = Button(win)
btn.config(text = "입력", command=msg)
btn.pack()
...
paper=cv2.imread('papersnow.jpg')     #종이의 역할을 함 / 가로 500,세로700 

#반복문을 이용한 이미지 데이터화하기 cutting
#초성 이미지 배열 데이터화  //(세로 100,가로1900)으로 100*100씩 잘라서 오른쪽으로 이동하며 데이터화
for i in range(20):
    cutting=cv2.imread('korean/firstsum.jpg')
    x=i*100
    y=0
...
for j in range(21):
    if j<9:
        cutting=cv2.imread('korean/heightmid.jpg')  #세로모음
        x=j*100
        y=0
        midim[j]=cutting[y:y+200,x:x+100]    
    elif j<14 :
        cutting=cv2.imread('korean/widthmid.jpg')   #가로모음
        x=(j-9)*200
        y=0
...
for h in range(27):
    cutting=cv2.imread('korean/lastsum.jpg')
    x=h*100
    y=0
...
for letter in range(size):                 #letter 글자단위 유닛
    ...
    size_jamo=len(jamo_str)                #jamo_str[0]=ㅂ //jamo_str[1]=ㅏ//jamo_str[2]=ㅇ #size_jamo=3
    ...
    num1=first.index(jamo_str[0],0,20)
    if size_jamo>1:
    ...
    #글자 사이즈를 축소함
    resize = cv2.resize(result, dsize=(0, 0), fx=0.2, fy=0.2)
    #result 변수에 반복문 한번에 한글자의 이미지가 할당됨. 이를 paper에 위치 지정을 하여 합성
    papercut=paper[up:up+50,left:left+40]
    paper[up:up+50,left:left+40]= cv2.bitwise_and(papercut,papercut,mask=masking(resize))
